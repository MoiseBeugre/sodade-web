...
export default function ContractPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const contractRes = await fetch(`https://sodade-api-production.up.railway.app/groups/${groupData.id}/contract`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSign = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`https://sodade-api-production.up.railway.app/groups/${group.id}/contract/sign`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const contractRes = await fetch(`https://sodade-api-production.up.railway.app/groups/${group.id}/contract`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const contractData = await contractRes.json();
      setContract(contractData);
